...
class HexCalc {
    ...
    public String decimalToHexadecimal(int decimalNumber) {
        ...
        List<Character> hexadecimalSeries = Arrays.asList(
    ...
    public int hexadecimalToDecimal(String hexadecimalNumber) {
        ...
        for (int i = hexadecimalNumberToConvert.length - 1; i >= 0; i--) {
            decimalNumber = decimalNumber + hexadecimalSeries.indexOf(hexadecimalNumberToConvert[order])
                    * (int) (Math.pow(hexCoefficient, i));
            order += 1;
        }
